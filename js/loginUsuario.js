...
var usuarioActualFactura = 0;
var usuarioMostrarFactura = [];
...
function login(){
	...
	for (var i = 0; i < login.length; i++) {
		if(usuarioActual == login[i].nombreUsuario){
			usuarioActualFactura = i;
			usuarioMostrarFactura.push(usuarioActualFactura);
			window.localStorage.setItem('usuarioMostrarFactura', JSON.stringify(usuarioMostrarFactura));
		}
	}
